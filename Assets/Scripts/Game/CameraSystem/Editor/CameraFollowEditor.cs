...
{
    private BindingFlags _bindFlags = BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static;
    ...
    private void ShowOnEnum(string enumFieldName, string enumValue, string fieldName)
    {
        ...
        {                    
            ...
            var enumNames = currentEnumValue.GetType().GetFields(_bindFlags);
        ...
        if (newFieldCondition.p_isValid)
        {
            FieldInfo fieldWithCondition = target.GetType().GetField(fieldName, _bindFlags);
        ...
        if (obj.NextVisible(true))
        {
            ...
            do
            {
                ...
                foreach (var fieldCondition in fieldConditions)
                {
                    ...
                    {
                        FieldInfo enumField = target.GetType().GetField(fieldCondition.p_enumFieldName, bindFlags);
                        var currentEnumValue = enumField.GetValue(target);
